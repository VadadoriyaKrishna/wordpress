...
# Database connection
conn = M.connect(host='localhost', username='phpmyadmin', password='root123', database='email_bifurcation')
curr = conn.cursor()
conn.commit()
...
def fetch_emails():
    ...
    for i, msg in enumerate(mailBox.fetch(criteria=AND(date=now.date())), start=1):
        ...
        for att in msg.attachments:                             #attachment save in a local
            ...
            filename_without_spaces_dashes = attachment_filename.replace(' ','').replace('-','')
            current_date = datetime.now().strftime('%Y%m%d')
            new_filename = f"{os.path.splitext(filename_without_spaces_dashes)[0]}_{current_date}{os.path.splitext(filename_without_spaces_dashes)[1]}"

            print("file name without space")
            print(filename_without_spaces_dashes)
            print('***********')
...
#        type_of_mail = GetPredictionByModel(mail_text)       # function fetch from mail_bifurcation.py
        ...
        val = (email["Sender_id"], email["Reciever_id"][0], type_of_mail, flag, ','.join(email["attachments"]), email["date"], time)# receiver id return one tuple like bc /bcc
